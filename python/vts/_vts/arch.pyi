from __future__ import annotations

from typing import Literal

_ComponentClassStr = Literal["lut", "LUT", "latch", "LATCH", "ff", "FF"]

_PortKindStr = Literal[
    "i", "I", "in", "IN", "input", "input", "o", "O", "out", "OUT", "output", "OUTPUT"
]

_PortClassStr = Literal[
    "lut_in",
    "LUT_IN",
    "lut_out",
    "LUT_OUT",
    "latch_in",
    "LATCH_IN",
    "latch_out",
    "LATCH_OUT",
    "ff_in",
    "FF_IN",
    "ff_out",
    "FF_OUT",
]

_ConnectionKindStr = Literal["direct", "DIRECT", "complete", "COMPLETE", "mux", "MUX"]

class PyComponentClass:
    LUT = ...
    LATCH = ...

class PyPortKind:
    INPUT = ...
    OUTPUT = ...

class PyPortClass:
    LUT_IN = ...
    LUT_OUT = ...
    LATCH_IN = ...
    LATCH_OUT = ...

class PyConnectionKind:
    DIRECT = ...
    COMPLETE = ...
    MUX = ...

class PyModule:
    def __new__(cls, name: str) -> PyModule: ...
    def name(self) -> str: ...
    def copy(self, name: str | None = None) -> PyModule: ...
    def add_component(
        self,
        name: str | PyComponent | None = None,
        *,
        component: PyComponent | None = None,
        class_: PyComponentClass | _ComponentClassStr | None = None,
    ) -> PyComponent: ...

class PyComponent:
    def module(self) -> PyModule: ...
    def name(self) -> str: ...
    def class_(self) -> PyComponentClass: ...
    def add_port(
        self,
        name: str | PyPort | None = None,
        *,
        port: PyPort | None = None,
        kind: PyPortKind | _PortKindStr | None = None,
        n_pins: int | None = None,
        class_: PyPortClass | _PortClassStr | None = None,
    ) -> PyPort: ...
    def add_reference(
        self,
        component: PyComponent,
        *,
        alias: str | None = None,
        n_instances: int | None = None,
    ) -> PyComponentRef: ...
    def add_connection(
        self,
        source_pins: PyPortPins,
        sink_pins: PyPortPins,
        source_component: PyComponentRef | None = None,
        sink_component: PyComponentRef | None = None,
        *,
        kind: PyConnectionKind | _ConnectionKindStr | None = None,
    ) -> None: ...
    def __getattr__(self, port: str) -> PyPort: ...
    def __setattr__(self, source: str, sink: PyPortSelection) -> None: ...

class PyPort:
    def module(self) -> PyModule: ...
    def name(self) -> str: ...
    def kind(self) -> PyPortKind: ...
    def n_pins(self) -> int: ...
    def class_(self) -> PyPortClass: ...
    def select(self, index: slice | int) -> PyPortPins: ...
    def __getitem__(self, index: slice | int) -> PyPortSelection: ...

class PyComponentRef:
    def module(self) -> PyModule: ...
    def component(self) -> PyComponent: ...
    def alias(self) -> str | None: ...
    def alias_or_name(self) -> str: ...
    def n_instances(self) -> int: ...
    def __getattr__(self, port: str) -> PyComponentRefPort: ...
    def __setattr__(self, source: str, sink: PyPortSelection) -> None: ...

class PyPortPins:
    pass

class PyComponentRefPort:
    def __getitem__(self, index: slice | int) -> PyPortSelection: ...
    def __setitem__(
        self, index: slice | int, sink: PyPortSelection | PyComponentRefPort
    ) -> None: ...

class PyPortSelection:
    pass

def json_dumps(module: PyModule, pretty: bool) -> str: ...
def json_loads(input: str) -> PyModule: ...
def yaml_dumps(module: PyModule) -> str: ...
def yaml_loads(input: str) -> PyModule: ...
def toml_dumps(module: PyModule, pretty: bool) -> str: ...
def toml_loads(input: str) -> PyModule: ...
