...
macro_rules! iter_dict_items {
    (for ( $key:ident : $key_ty:ty [ $key_conv:ident ] , $val:ident : $val_ty:ty [ $val_conv:ident ] ) in $dict:expr => $runner:expr) => {
        for ($key, $val) in $dict.iter() {
            let $key = $key.$key_conv::<$key_ty>()?;
            let $val = $val.$val_conv::<$val_ty>()?;
            $runner
        }
    };
}
...
macro_rules! iter_mapping_items {
    (for ( $key:ident : $key_ty:ty [$key_conv:ident], $val:ident : $val_ty:ty [$val_conv:ident] ) in $mapping:expr => $runner:expr) => {
        let items = $mapping.items()?.to_list()?;
        for item in items.iter() {
            let item = item.downcast::<pyo3::types::PyTuple>()?;
            let $key = item.get_borrowed_item(0)?;
            let $key = $key.as_any().$key_conv::<$key_ty>()?;
            let $val = item.get_borrowed_item(1)?;
            let $val = $val.as_any().$val_conv::<$val_ty>()?;
            $runner
        }
    };
}
